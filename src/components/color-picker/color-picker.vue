  ...
  >
    ...
    <template #default>
      <div
        ...
        @blur="handleBlur"
      >
        <div v-if="$slots.default" class="el-color-picker-content" @click="handleTrigger">
          <slot></slot>
        </div>
        <div v-else="colorDisabled && !$slots.default" :class="ns.be('picker', 'mask')" />
        <div
          v-if="!$slots.default"
...
import { useFocusController, useLocale, useNamespace } from "element-plus";
import { EVENT_CODE, UPDATE_MODEL_EVENT } from "element-plus";
...
const { isFocused, handleFocus: _handleFocus, handleBlur } = useFocusController(
  triggerRef,
  {
    beforeBlur(event) {
